...
export async function getCurrentWeatherData(lat,long) {
  const url = `${WeatherApiConfig.baseUrl + WeatherApiConfig.currentWeather}?key=${WeatherApiConfig.apiKey}&q=${lat},${long}`
...
export async function getForecastData(lat,long) {
  const url = `${WeatherApiConfig.baseUrl + WeatherApiConfig.forecast}?key=${WeatherApiConfig.apiKey}&q=${lat},${long}&days=7`
